...
- [x] 3.1 Unit tests pass: `cd backend && uv run pytest tests/cabinet` — 75e02c7
- [x] 3.2 Integration tests pass (native PowerShell): `cd backend; uv run pytest tests/integration/cabinet` — 75e02c7
- [x] 3.3 Lint + format clean: `cd backend && uv run ruff check . && uv run ruff format --check .` — 75e02c7

#### Manual

- [x] 3.4 PATCH package_count: 0 succeeds and persists at 0 — 75e02c7
- [x] 3.5 Negative count → 422; out-of-range/non-tablet partial → 422 — 75e02c7
- [x] 3.6 Response reflects recomputed status/below_minimum/total_tablets — 75e02c7
- [x] 3.7 PATCH on foreign/non-existent entry → 404 — 75e02c7

### Phase 4: Frontend — quantity steppers + zero rule

#### Automated

- [x] 4.1 Build passes: `cd frontend && npm run build` — 5c8e192
- [x] 4.2 Lint passes: `cd frontend && npm run lint` — 5c8e192
- [x] 4.3 Format clean: `cd frontend && npx prettier --check src/` — 5c8e192

#### Manual

- [x] 4.4 −/+ adjusts package count; list refetches; badge/status update — 5c8e192
- [x] 4.5 Partial-tablet edit/clear works for tablet entries, hidden for non-tablet — 5c8e192
- [x] 4.6 Uncategorised entry 1 → 0 confirms then deletes; cancel leaves at 1 — 5c8e192
- [x] 4.7 Important/used entry decrements to 0 and stays (no dialog) — 5c8e192
- [x] 4.8 Rapid −/+ clicks lose no decrements; steppers disable while a mutation is in flight — 5c8e192
- [x] 4.9 Consistent on desktop row and mobile card — 5c8e192

### Phase 5: E2E — manage/delete spec + teardown

#### Automated

- [x] 5.1 Manage/delete spec passes (native PowerShell): `cd frontend; npx playwright test manage-cabinet-entry` — eea956d
- [x] 5.2 Full e2e suite still green: `cd frontend; npx playwright test` — eea956d
- [x] 5.3 Lint + format clean on the new spec: `cd frontend && npm run lint && npx prettier --check e2e/` — eea956d

#### Manual

- [x] 5.4 Playwright report shows the manage/delete journey passing — eea956d
- [x] 5.5 No orphaned test entries remain after a run (teardown worked) — eea956d
