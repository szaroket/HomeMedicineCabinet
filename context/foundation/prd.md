...
- FR-008: User receives email notifications at their configured frequency while a medication is within the expiry threshold window; when fewer than 7 days remain until expiry, the system automatically escalates to daily notifications regardless of the user's frequency setting. Priority: must-have
  > Socrates: Counter-argument considered: "email requires a background scheduler and email delivery service, adding infrastructure cost." Resolution: kept — email is the only viable async notification channel for a web-only app; infrastructure cost is accepted.
...
- FR-012: Each cabinet entry displays links to the drug leaflet and specification sourced from the official dataset. Priority: must-have. Inline document preview: nice-to-have (v2).
...
- One user's cabinet data must never be visible to another user (strict data isolation between accounts).
...
- Role model: flat single-role — all registered accounts are equal users; no role separation in MVP.
- No admin UI in MVP.
- Unauthenticated users cannot access any cabinet data or application features beyond the registration and login screens.
...
- No photo, barcode, or document scanning for adding medications — manual entry via the approved list only.
- No inline document preview for MVP — leaflet and specification links are provided as plain links; inline preview deferred to v2.

## Open Questions

1. **Fallback deadline**: the target hard deadline is 2026-07-05. A fallback deadline of 2026-08-10 was noted during shaping. If the primary deadline is missed, the fallback is the latest acceptable ship date. Owner: user. No blocking impact on PRD.
