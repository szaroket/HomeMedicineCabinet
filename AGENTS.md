...
Docstrings follow the **Google style** convention (`Args:`/`Returns:`/`Raises:` sections), enforced by ruff's pydocstyle rules (`convention = "google"` in `backend/pyproject.toml`). Docstrings are not required everywhere (`D1` rules are ignored), but any docstring you write must follow Google formatting. Alembic migrations under `backend/migrations/` are exempt. All `Args:`, `Attributes:`, `Returns:`, and `Yields:` entries must include type hints. Parameters and attributes use parenthesised types: `name (type): description`. Return and yield values use a bare type prefix without parentheses: `type: description`.
